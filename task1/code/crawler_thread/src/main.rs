...
fn save_response(name: String, body: String) -> Result<()>{
    let result_save_dir = "websiteshtml/";
    let _ = create_dir_all(result_save_dir);
...
fn crawl(name: String, url: Url, client: Client, sender: mpsc::Sender<(String, String)>) -> Result<u128> {
    let now = Instant::now();
    ...
    if let Err(_) = sender.send((name.clone(), body)) {
    ...
    println!("{} delay_millis = {}  mem_use = {} vmem_use = {}", name, delay_millis, mem_use, vmem_use);
    Ok(delay_millis)
}
...
fn main() ->  Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, url) in websites {
        ...
        let handle = thread::spawn(move || {
            match crawl(name, url, client_clone, tx_clone) {
                Ok(delay_time) => {
                    //println!("调试：爬取成功一次！");
                    let mut c = count_clone.lock().unwrap();
                    *c += 1;
                    let mut d = delay_times_clone.lock().unwrap();
                    d.push(delay_time);
                },
                Err(_) => {
                    //println!("爬取网站失败一次: {}", e);
    ...
    while let Ok((name, body)) = rx.recv() {
        save_response(name, body)?;
    }
    ...
    if final_count == 0 {
        ...
        let mut delay_times = delay_times.lock().unwrap();
        delay_times.sort();
